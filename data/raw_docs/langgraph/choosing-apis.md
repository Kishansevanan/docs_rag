...
const searchNode: GraphNode = async (state) => {
  // Access shared state
  const results = await search(state.userInput);
  return { searchResults: results };
};

const validationNode: GraphNode = async (state) => {
  // Access results from previous node
...
// Simple data processing using Functional API
const dataProcessor = entrypoint({}, async (rawData: Record) => {
...
// Use the functional API result in the graph
const orchestratorNode: GraphNode = async (state) => {
...
const shouldAnalyze: ConditionalEdgeRouter = (state) => {
  return state.step1Result?.needsAnalysis ? "analyze" : "simplePath";
};

const confidenceCheck: ConditionalEdgeRouter = (state) => {
