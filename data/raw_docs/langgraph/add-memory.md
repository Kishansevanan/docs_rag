  ...
  ```
    ...
    **Setup**
    To use the [MongoDB checkpointer](https://pypi.org/project/langgraph-checkpoint-mongodb/), you will need a MongoDB cluster. Follow [this guide](https://www.mongodb.com/docs/guides/atlas/cluster/) to create a cluster if you don't already have one.
      ...
      from langchain.chat_models 
      from langgraph.graph 
      from langgraph.checkpoint.mongodb 
...
```
      ...
      from langchain.chat_models 
      from langgraph.graph 
      from langgraph.checkpoint.mongodb.aio 
...
```
      ...
      from langchain.chat_models 
      from langgraph.graph 
      from langgraph.checkpoint.redis.aio 
...
:::
  ...
  ```
    ...
    You need to call `store.setup()` the first time you're using Postgres store
      ...
      from langgraph.graph 
      from langgraph.checkpoint.postgres.aio 
      from langgraph.store.postgres.aio 
      from langgraph.runtime 
...
:::python
  ...
  ```
    ...
    You need to call `store.setup()` the first time you're using [Redis store](https://pypi.org/project/langgraph-checkpoint-redis/).
      ...
      from langgraph.graph 
      from langgraph.checkpoint.redis.aio 
      from langgraph.store.redis.aio 
      from langgraph.runtime 
...
:::
    ...
    const chat: GraphNode = async (state, runtime) => {
      // Search based on user's last message
...
```
  ...
  const deleteMessages: GraphNode = (state) => {
    const messages = state.messages;
    if (messages.length > 2) {
      // remove the earliest two messages
...
const summarizeConversation: GraphNode = async (state) => {
  // First, we get any existing summary
