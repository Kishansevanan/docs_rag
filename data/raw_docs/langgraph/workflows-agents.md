...
// First LLM call to generate initial joke
const generateJoke: GraphNode = async (state) => {
  const msg = await llm.invoke(`Write a short joke about ${state.topic}`);
  return { joke: msg.content };
};

// Gate function to check if the joke has a punchline
const checkPunchline: ConditionalEdgeRouter = (state) => {
  // Simple check - does the joke contain "?" or "!"
...
// Second LLM call to improve the joke
const improveJoke: GraphNode = async (state) => {
...
// Third LLM call for final polish
const polishJoke: GraphNode = async (state) => {
  const msg = await llm.invoke(
...
// Nodes
// First LLM call to generate initial joke
const callLlm1: GraphNode = async (state) => {
  const msg = await llm.invoke(`Write a joke about ${state.topic}`);
  return { joke: msg.content };
};

// Second LLM call to generate story
const callLlm2: GraphNode = async (state) => {
  const msg = await llm.invoke(`Write a story about ${state.topic}`);
  return { story: msg.content };
};

// Third LLM call to generate poem
const callLlm3: GraphNode = async (state) => {
  const msg = await llm.invoke(`Write a poem about ${state.topic}`);
  return { poem: msg.content };
};

// Combine the joke, story and poem into a single output
const aggregator: GraphNode = async (state) => {
...
// Nodes
// Write a story
const llmCall1: GraphNode = async (state) => {
...
// Write a joke
const llmCall2: GraphNode = async (state) => {
  const result = await llm.invoke([{
    role: "system",
...
// Write a poem
const llmCall3: GraphNode = async (state) => {
  const result = await llm.invoke([{
    role: "system",
...
const llmCallRouter: GraphNode = async (state) => {
  // Route the input to the appropriate node
...
// Conditional edge function to route to the appropriate node
const routeDecision: ConditionalEdgeRouter = (state) => {
  // Return the node name you want to visit next
  if (state.decision === "story") {
    return "llmCall1";
...
const llmCall = task("sectionWriter", async (section: z.infer) => {
  // Generate section
  const result = await llm.invoke([
    {
      role: "system",
...
// Nodes
const orchestrator: GraphNode = async (state) => {
  // Generate queries
  const reportSections = await planner.invoke([
    { role: "system", content: "Generate a plan for the report." },
...
const synthesizer: GraphNode = async (state) => {
  // List of completed sections
...
// Conditional edge function to create llm_call workers that each write a section of the report
const assignWorkers: ConditionalEdgeRouter = (state) => {
  // Kick off section writing in parallel via Send() API
