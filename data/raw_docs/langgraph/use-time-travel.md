...
3. [Update the graph state (optional)](#3-update-the-state-optional): Use the @[`update_state`] method to modify the graph's state at the checkpoint and resume execution from alternative state.
...
const generateTopic: GraphNode = async (state) => {
  // LLM call to generate a topic for the joke
