...
flowchart TD
    %% Start
    Start([💬 Customer reportsan issue]) --> Warranty{Is the deviceunder warranty?}

    %% Warranty check
    Warranty -->|✅ Yes| IssueType{What typeof issue?}
    Warranty -->|❌ No| OutOfWarranty{What typeof issue?}

    %% In-Warranty branch
    IssueType -->|🔩 Hardware| Repair[Provide warrantyrepair instructions]
    IssueType -->|💻 Software| Troubleshoot[Provide troubleshootingsteps]

    %% Out-of-Warranty branch
    OutOfWarranty -->|🔩 Hardware| Escalate[Escalate to humanfor paid repair options]
