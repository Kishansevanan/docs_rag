...
type ResponseFormat = (
    ...
    | StandardSchema // any Standard Schema library
    | Record // a JSON Schema
)
...
function providerStrategy(
    schema: ZodSchema | SerializableSchema | JsonSchemaFormat
): ProviderStrategy
```
...
```
    ...
    from typing 
    from langchain.agents 
    from langchain.agents.structured_output 
...
function toolStrategy(
    responseFormat:
        ...
        | ZodSchema
        | SerializableSchema
        | (ZodSchema | SerializableSchema | JsonSchemaFormat)[]
...
```
    ...
    - **`False`**: No retry, let exceptions propagate
    - **`(error: ToolStrategyError) => string | Promise`**: retry with the provided message or throw the error
