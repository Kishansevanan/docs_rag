...
async function* sttStream(
  ...
  const passthrough = writableIterator();
...
export class AssemblyAISTT {
  ...
  async sendAudio(buffer: Uint8Array): Promise {
  ...
  protected get _connection(): Promise {
...
export class CartesiaTTS {
  ...
  async sendText(text: string | null): Promise {
...
app.get("/ws", upgradeWebSocket(async () => {
  const inputStream = writableIterator();
