...
flowchart TD
    Start([💬 User: Write SQL queryfor high-value customers]) --> SystemPrompt[📋 Agent sees skill descriptions:• sales_analytics• inventory_management]

    SystemPrompt --> Decide{🤔 Need sales schema}

    Decide --> LoadSkill[🔧 load_skill'sales_analytics']

    LoadSkill --> Schema[📊 Schema loaded:customers, orders tables+ business logic]

    Schema --> WriteQuery[✍️ Agent writes SQL queryusing schema knowledge]

    WriteQuery --> Response([✅ Returns valid SQLfollowing business rules])
...
# Example: from langchain_anthropic 
